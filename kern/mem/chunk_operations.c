...
//		return;
	/*====================================*/
//	cprintf("Hiiii inside alloc_usr_mem\n");
#if USE_KHEAP
	...
	for(int i = 0;i < needed_pages; ++i){
		uint32* my_env_table;
		int pt_exists = get_page_table(e->env_page_directory,virtual_address,&my_env_table);

		if(pt_exists == TABLE_NOT_EXIST){
			my_env_table = create_page_table(e->env_page_directory,virtual_address);
		}
		my_env_table[PTX(virtual_address)] |= PERM_UHPAGE; // this page is now marked as used in PT (9th bit)
		// fault handler will actually allocate this page in memory if BIT[0x400] =  1

		virtual_address += PAGE_SIZE; // get to the next page to mark if there's any more
	}
#endif
//	cprintf("you shouldn't be here in alloc_usr_mem\n");
}
...
//		return;
	...
	for(int i = 0;i < needed_pages; ++i){
		...
		if(pt_exists == TABLE_NOT_EXIST){
			return; // no PT to free memory from !!
		}
		my_env_table[PTX(virtual_address)] &= ~PERM_UHPAGE; // page's BIT[0x400] = unmarked

		env_page_ws_invalidate(e,virtual_address); // remove from env's working set
		pf_remove_env_page(e,virtual_address); // remove from disk
