...
inline int pt_get_page_permissions(uint32* directory, uint32 virtual_address )
{
	...
	uint32* cur_page_table ;
    get_page_table(directory, virtual_address, &cur_page_table);
	// panic("pt_get_page_permissions() is not implemented yet!");
    if(cur_page_table == NULL){
    	panic("there is no page table ");
    }
    return cur_page_table[PTX(virtual_address)];
}
