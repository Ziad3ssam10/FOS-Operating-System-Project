...
//	panic("create_user_kern_stack() is not implemented yet...!!");
	...
	//	pt_set_page_permissions(ptr_user_page_directory, (uint32) st, ~PERM_PRESENT,PERM_PRESENT);
		uint32* cur_table;
		get_page_table(ptr_user_page_directory, (uint32)st, &cur_table);
		if (cur_table == NULL) {
			panic("there to page table ");
		}
	cur_table[PTX(st)] &= ~PERM_PRESENT;
		return st;
	}
#else
